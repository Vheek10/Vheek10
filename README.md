<h1 align="center"> I'm Victor Gp</h1>
<h3 align="center">Full-Stack & Smart Contract Engineer 🚀</h3>
<p align="center">I build futuristic web experiences that bridge web2 and web3 — from sleek Next.js apps to secure smart contracts.</p>

<p align="center">
  <a href="https://victorgp.vercel.app">
    <img src="https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white" />
  </a>
  <a href="https://linkedin.com/in/vheek10">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://x.com/Vheek10">
    <img src="https://img.shields.io/badge/Twitter-1DA1F2?style=for-the-badge&logo=x&logoColor=white" />
  </a>
  <a href="mailto:jeremiahvictorgp@gmail.com">
    <img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
</p>

---

## 🌟 About Me

I’m a developer who turns imagination into lines of elegant, production-ready code. Whether I’m crafting a responsive React interface or deploying audited smart contracts, I’m driven by clarity, innovation, and connecting systems that scale.  

---

## 💼 Experience

**Lead Developer · ValueContentAfrica** (2025 – Present)  
- Built a full-stack platform empowering African creators  
- Designed a microservices backend for real-time analytics  
- Led implementation strategy and team delivery  

**Full-Stack & Smart Contract Engineer · StrataDeed** (2025 – Present)  
- Built privacy-centric ZK protocols for real estate tokenization  
- Improved gas efficiency by 65% through contract design  
- Developed frontend with Next.js, TypeScript & modern UX  

**Freelance Developer** (2024 – Present)  
- Delivered 20+ full-stack & web3 projects  
- Delivered ELGVotes, custom explorers, and portfolio sites  
- Solved real-world problems with clean, scalable solutions  

<h2 align="center">🛠 Tech Stack</h2>

<table align="center" border="0" cellpadding="12" cellspacing="0">
  <tr>
    <td align="center" width="120" style="border:none;">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nextjs/nextjs-original.svg" width="40"/><br/>
      <b>Next.js</b>
    </td>
    <td align="center" width="120" style="border:none;">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" width="40"/><br/>
      <b>React</b>
    </td>
    <td align="center" width="120" style="border:none;">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg" width="40"/><br/>
      <b>TypeScript</b>
    </td>
    <td align="center" width="120" style="border:none;">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" width="40"/><br/>
      <b>JavaScript</b>
    </td>
  </tr>

  <tr>
    <td align="center" width="120" style="border:none;">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" width="40"/><br/>
      <b>HTML</b>
    </td>
    <td align="center" width="120" style="border:none;">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" width="40"/><br/>
      <b>CSS</b>
    </td>
    <td align="center" width="120" style="border:none;">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tailwindcss/tailwindcss-original.svg" width="40"/><br/>
      <b>Tailwind</b>
    </td>
    <td align="center" width="120" style="border:none;">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/solidity/solidity-original.svg" width="40"/><br/>
      <b>Solidity</b>
    </td>
  </tr>

  <tr>
    <td align="center" width="120" style="border:none;">
      <img src="https://raw.githubusercontent.com/ethereansos/brand-assets/main/foundry/foundry-logo.png" width="40"/><br/>
      <b>Foundry</b>
    </td>
    <td align="center" width="120" style="border:none;">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mongodb/mongodb-original.svg" width="40"/><br/>
      <b>MongoDB</b>
    </td>
    <td align="center" width="120" style="border:none;">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" width="40"/><br/>
      <b>PostgreSQL</b>
    </td>
    <td align="center" width="120" style="border:none;">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/firebase/firebase-plain.svg" width="40"/><br/>
      <b>Firebase</b>
    </td>
  </tr>

  <tr>
    <td align="center" width="120" style="border:none;">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/supabase/supabase-original.svg" width="40"/><br/>
      <b>Supabase</b>
    </td>
    <td style="border:none;"></td>
    <td style="border:none;"></td>
    <td style="border:none;"></td>
  </tr>
</table>


## 🏆 Featured Projects

**🎯 StrataDeed · Real Estate Tokenization**  
Tech: Next.js · TypeScript · Solidity · Mantle Network · ZK Proofs  
Builds a privacy-first fractional ownership platform.  
[Live Demo](https://strata-deed.vercel.app) · [GitHub](https://github.com/stratadeed)

**🌍 ValueContentAfrica · Creator Platform**  
Tech: Next.js · Node.js · PostgreSQL · TailwindCSS  
End-to-end platform for content creators with analytics.  
[Live Demo](https://value-content-africa.vercel.app) · [GitHub](https://github.com/vheek10/valuecontentafrica)

**📊 PolySight · Solana Prediction Market**  
Tech: Next.js · Wagmi · Rust · TailwindCSS  
A prediction market interface for Solana ecosystems.  
[Live Demo](https://polysight.vercel.app) · [GitHub](https://github.com/vheek10/polysight)

---

## 📊 GitHub Stats

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Vheek10&show_icons=true&theme=dark&hide_border=true&count_private=true&bg_color=0D1117" width="48%" alt="Victor's GitHub Stats" />
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=Vheek10&theme=github-dark&hide_border=true&bg_color=0D1117&color=58A6FF&line=58A6FF" width="48%" alt="Activity Graph" />
</div>

---

## 🔤 Most Used Languages

<div align="center">
  <!-- dynamically shows your top languages -->
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Vheek10&layout=compact&theme=dark&hide_border=false&bg_color=0D1117" alt="Top Languages" />
</div>


---

## 🔒 Smart Contract Specialties

- DeFi: DEX, lending, yield optimizers  
- NFTs: ERC-721/1155, marketplaces, royalties  
- Governance: DAOs, voting, treasury frameworks  
- Security: audit prep, gas optimization, test coverage

---

## 🚀 How I Build

🛠️ **Plan** → ⚙️ **Develop** → 🔍 **Test** → 🚀 **Deploy**  
- From architecture and UX to smart contracts  
- Iterative testing, audits, and performance tuning  
- CI/CD and monitoring for reliable production

---

## 🤝 Let’s Build

Whether it’s a full dApp, smart contract suite, or API-driven web experience, I’m ready.  
📬 Reach out for contract work, consulting, or collaborations.

*Let’s make ideas reality!*
